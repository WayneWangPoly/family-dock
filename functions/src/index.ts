...
function parseLocalFallbackAction(transcript: string, uid: string) {
  ...
  const hasPlaceKeyword =
    /location|place|address|school|club|home/i.test(text) ||
    text.includes("\u5730\u70b9") ||
    text.includes("\u5730\u5740") ||
    text.includes("\u4f4d\u7f6e") ||
    text.includes("\u5b66\u6821") ||
  ...
  const hasAddKeyword =
    /add|create|save|new/i.test(text) ||
    text.includes("\u6dfb\u52a0") ||
    text.includes("\u65b0\u589e") ||
    text.includes("\u589e\u52a0") ||
    text.includes("\u52a0\u5165") ||
  ...
  const extractValueAfter = (keywords: string[]) => {
    for (const keyword of keywords) {
      ...
      if (index >= 0) {
        const raw = text
          ...
          .replace(/^[:：,\s]+/, "")
          .trim();
        if (raw) return cleanAiText(raw.split(/[。；;\n]/)[0] ?? raw);
      }
    }
    return "";
  };
  ...
  if (hasPlaceKeyword && hasAddKeyword) {
    let address =
      extractValueAfter(["address", "located at", "at"]) ||
      extractValueAfter(["\u5730\u5740", "\u4f4d\u7f6e"]);

    let name =
      extractValueAfter(["name", "named", "called"]) ||
    ...
    const placeType =
      lower.includes("school") || text.includes("\u5b66\u6821")
        ? "school"
  ...
  const isRequest =
    (/request|ask/i.test(text) || text.includes("\u7533\u8bf7")) &&
...
export const parseAiCommand = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      parsed = fallback ?? {
        ...
        clarifying_question:
          "AI is connected, but the model response is not ready. Try adding clear details such as name, date, time, address or amount.",
    ...
    const actionMissingFields = normalizedActions.flatMap((action) =>
      Array.isArray(action.missing_fields) ? action.missing_fields : [],
    );
    const missingFields =
      actionMissingFields.length > 0
    ...
    const normalized = {
      ...
      clarifying_question:
        normalizedActions.length > 0 && missingFields.length === 0
          ? null
...
export {
  ...
  summarizeLearning,
  undoFamilyAction,
} from "./firebaseMigrationAdditions.js";
...
} from "./firebaseMigrationPass2.js";
