...
export const transcribeAudio = onCall({ region: "us-central1", secrets: [openAiApiKey] }, async (request) => {
    ...
    const prompt = safeText(request.data?.prompt, "Family schedule, child activities, homework, locations, requests, payments, fencing, school, Adelaide.");
    try {
        ...
        throw new HttpsError("internal", `Audio transcription failed: ${String(error?.message ?? error).slice(0, 300)}`);
    }
});
...
function minutesBeforeIso(iso, minutes) {
    const time = new Date(iso).getTime();
    if (!Number.isFinite(time))
        return null;
    return new Date(time - minutes * 60 * 1000).toISOString();
}
function routeDocIdPart(value) {
...
async function loadPlaceMap(familyId, placeIds) {
...
async function buildDailyRouteDeparturePlansInternal(args) {
    ...
    const rawEvents = eventsSnap.docs
        ...
        .filter((event) => {
        if (!event.start_at || !event.place_id)
            return false;
        if (event.start_at.slice(0, 10) !== date)
            return false;
        if (["cancelled", "canceled", "deleted"].includes(event.status.toLowerCase()))
            return false;
        if (args.childId && event.child_id !== args.childId)
            return false;
        return true;
    });
    ...
    for (const [childId, events] of byChild.entries()) {
        ...
        for (let index = 1; index < placeEvents.length; index += 1) {
            ...
            if (fromEvent.place_id === toEvent.place_id)
                continue;
            ...
            if (!latestSafeDepartureAt || !recommendedDepartureAt)
                continue;
            legs.push({
                index,
                fromEvent,
...
function futureDepartureTime(value) {
...
async function refreshRouteLegTravelTimesInternal(args) {
    ...
    let query = db.collection(`families/${args.familyId}/route_departure_legs`);
    if (planId) {
...
export const refreshRouteLegTravelTimes = onCall({ region: "us-central1", secrets: [googleMapsApiKey] }, async (request) => {
    const uid = assertAuthed(request.auth?.uid);
    const familyId = cleanFamilyId(request.data?.family_id);
    await assertFamilyMember(familyId, uid);
...
function reminderIsInactiveStatus(value) {
    const status = safeText(value).toLowerCase();
    return ["cancelled", "canceled", "deleted", "archived", "done", "completed", "paid"].includes(status);
}
function reminderStatusText(value, fallback = "scheduled") {
    return safeText(value, fallback).toLowerCase();
}
async function hasRecentFamilyReminderNotification(familyId, sourceId, notificationType, cooldownMinutes = 360) {
    if (!sourceId)
        return false;
...
async function buildFamilyReminderCandidates(familyId) {
    const candidates = [];
    ...
    for (const docSnap of eventsSnap.docs) {
        const row = docSnap.data() ?? {};
        if (reminderIsInactiveStatus(row.status))
            continue;
...
async function runFamilyReminderScan(familyId, senderUid) {
    ...
    for (const candidate of candidates) {
        const cooldownActive = await hasRecentFamilyReminderNotification(familyId, candidate.source_id, candidate.notification_type, candidate.cooldown_minutes);
        if (cooldownActive) {
            skippedCooldown += 1;
            continue;
        }
...
export const systemHealthCheck = onCall({
    ...
    const problems = [];
    async function countCollection(path, limitCount = 500) {
    ...
    async function safeCount(path, limitCount = 500) {
        try {
    ...
    const [activePushSnap, scheduledJobsSnap, placesSnap, todayEventsSnap, todayPlansSnap, todayLegsSnap, recentLogsSnap, recentNotificationsSnap,] = await Promise.all([
    ...
    const todayEventsWithPlace = todayEvents.filter((event) => safeText(event.place_id)).length;
    const todayLegs = todayLegsSnap.docs.map((docSnap) => ({ id: docSnap.id, ...(docSnap.data() ?? {}) }));
    const googleRefreshedLegs = todayLegs.filter((leg) => {
        return safeText(leg.estimate_method) === "google_routes_traffic_aware" || safeText(leg.routes_refresh_status) === "ok";
    }).length;
    let reminderCandidateSummary = { ok: true, count: 0, top: [], error: null };
    try {
        const candidates = await buildFamilyReminderCandidates(familyId);
...
function shouldRunScheduledSetting(setting, triggerName) {
...
function minutesUntilFrom(baseMs, value) {
    const iso = safeText(value);
    if (!iso)
        return null;
    const time = new Date(iso).getTime();
...
function buildScheduledRouteRiskMessage(risk, label, minutesToRecommended, minutesToLatestSafe) {
    if (risk === "late")
...
async function runScheduledLateRiskScan(familyId) {
    const now = new Date();
    ...
    for (const planDoc of planSnap.docs) {
        ...
        let worstRisk = "normal";
        let worstMessage = "Route timing looks OK.";
        let worstRecommended = null;
        let worstLatestSafe = null;
        let worstLegId = null;
        const riskRank = { normal: 1, low: 1, medium: 2, high: 3, late: 4 };
        for (const legDoc of legsSnap.docs) {
            const leg = legDoc.data() ?? {};
...
async function runScheduledFamilySettings(triggerName) {
    const snap = await db
